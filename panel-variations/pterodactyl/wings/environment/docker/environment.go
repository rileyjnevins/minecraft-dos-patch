...
// ContainerNetInterface returns the veth interface (as netlink.Link struct) affiliated
// with this container. It does this based on network namespaces, each container has a
// unique network namespace ID. This ID is used by docker and represents the namespace
// between the container's interface (eth0 within the container) and the virtual
// ethernet (veth) interface that exists outside the container.
func (e *Environment) ContainerNetInterface() (netlink.Link, error) {
	c, err := e.client.ContainerInspect(context.Background(), e.Id)
	if err != nil {
		return nil, errors.Wrap(err, "environment/docker: failed to inspect container")
	}

	nsid, err := netlink.GetNetNsIdByPid(c.State.Pid)
	if err != nil {
		return nil, errors.WrapIf(err, "environment/docker: failed to get network namespace ID")
	}

	ifaces, err := netlink.LinkList()
	if err != nil {
		return nil, errors.WrapIf(err, "environment/docker: error while getting all network interfaces")
	}

	for _, iface := range ifaces {

		// The interfaces match, return the current iface index
		if iface.Attrs().NetNsID == nsid {
			return iface, nil
		}
	}
	return nil, errors.WrapIf(err, "environment/docker: could not find network interface")
}